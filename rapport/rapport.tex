...
\label{sec:orge7fd821}
\subsection{Introduction}
\label{sec:org86bc8a7}
...
certaines fonctions jugées principales qui sont expliquées avec leurs \hyperref[orgd9ec182]{graphes
respectifs}. 

\subsection{Classe Cell\label{orgd7e507a}}
\label{sec:org5cbb84e}
...
possédant une certaine couleur (\hyperref[orgab5322d]{Color}).\\
...
(\hyperref[org64cb1eb]{Direction}) donnée et les valeurs la référence à sa cellule voisine dans la
direction.\\

\subsubsection{Méthodes}
\label{sec:orgce04630}

\begin{enumerate}
\item \texttt{setNeighbor: Cell*Direction -> ()} \label{org7bc7794}
\label{sec:orgb72a217}

Cette méthode prend en paramètre une cellule (\hyperref[orgd7e507a]{Cell}) et une direction
(\hyperref[org64cb1eb]{Direction}) où la cellule représente la cellule voisine a attribue dans la
...
\item \texttt{getNeighbor: Direction -> Cell} \label{org9d996f1}
\label{sec:org9a227b2}

Cette méthode prend en paramètre une direction (\hyperref[org64cb1eb]{Direction}) et renvoie la
...
Les diagonales sont récupérées dans l'Enum (\hyperref[org64cb1eb]{Direction}) par la méthode
...
\subsection{Classe Grid\label{orgabd99a1}}
\label{sec:org386fa81}
...
\subsubsection{Attributs}
\label{sec:orgdcd0781}

La classe \hyperref[orgabd99a1]{Grid} est composé d'un tableau de cellules (\hyperref[orgd7e507a]{Cell}) contenant les
...
\begin{enumerate}
\item \texttt{initGrid: () -> ()}
\label{sec:orgfec906a}

Voici la fonction d'initialisation de la grille. Elle crée un tableau 2D
de cellules (\hyperref[orgd7e507a]{Cell}), elle assigne à chaque cellule leurs voisins respectifs
(\textbf{UP}, \textbf{DOWN}, \textbf{RIGHT}, \textbf{LEFT}) grâce à la fonction \hyperref[org7bc7794]{setNeighbor: Cell *
Direciton ->()} de (\hyperref[orgd7e507a]{Cell}).\\
...
\subsection{Classe Player\label{orgf11fd79}}
\label{sec:orgd083969}
\subsubsection{Attributs}
\label{sec:orgfcc1a7f}
...
\subsubsection{Méthodes}
\label{sec:org4cbf9ba}
\begin{enumerate}
\item \texttt{initToken: int -> Token}
\label{sec:orgc9de324}
...
\subsection{Classe Game\label{org382d869}}
\label{sec:org6fc0279}
...
\item La grille/plateau (\hyperref[orgabd99a1]{Grid}) de jeu.
\end{itemize}
...
\subsubsection{Méthodes}
\label{sec:org09de66e}
\begin{enumerate}
\item \texttt{loadSave: String -> ()}
\label{sec:org00ca3c4}
...
\subsection{Classe Save\label{orgc4702f3}}
\label{sec:org6833d3d}
...
\label{sec:org4885820}

Nous avons implémenté 3 énumérations dans notre logiciel.

\subsubsection{Entity\label{org8f1a2fb}}
\label{sec:orgd66c23c}
